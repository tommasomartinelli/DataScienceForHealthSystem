...
total_infections_both <- modified_data %>%
  ...
  summarize(total_infections = sum(value, na.rm = TRUE))
...
# Calcolo della media tra le infezioni maschili e femminili
average_infections_both <- (mean_infections_male + mean_infections_female) / 2

# Stampa dei valori delle medie
cat("Media delle infezioni maschili:", mean_infections_male, "\n")
cat("Media delle infezioni femminili:", mean_infections_female, "\n")
cat("Totale delle infezioni 'Both sexes':", total_infections_both$total_infections, "\n")
cat("Media tra infezioni maschili e femminili:", average_infections_both, "\n")
...
hist(filtered_data_clean$value, breaks = 20, main = "Value distribution", xlab = "Value")
...
wilcox_test <- wilcox.test(value ~ sex, data = filtered_data_clean)
...
for (i in 1:(length(areas)-1)) {
  for (j in (i+1):length(areas)) {
    ...
    u_test_result <- wilcox.test(value ~ geographic_area, data = filtered_data_clean,
                                 subset = geographic_area %in% c(area1, area2))
    
    cat("Mann-Whitney U Test between", area1, "and", area2, "\n")
    print(u_test_result)
    cat("\n")
  }
}
...
for (i in 1:(length(areas)-1)) {
  for (j in (i+1):length(areas)) {
    ...
    corrected_p_value <- p.adjust(u_test_result$p.value, method = "bonferroni")
    
    cat("Mann-Whitney U Test between", area1, "and", area2, "\n")
    print(u_test_result)
    cat("Corrected p-value:", corrected_p_value, "\n")
    cat("\n")
  }
}
